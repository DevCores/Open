...
class Model_Auth extends Model
{
    ...
    public function infoAccServer($username, $password)
    {
        ...
        $username = $this->escape($username);
        return $this->db->query("SELECT * FROM account WHERE username = '$username' AND sha_pass_hash = UPPER(SHA1(CONCAT(UPPER('$username'),':',UPPER('$password'))))")->fetch_assoc();
    }
    ...
    public function voteTime($user, $id)
    {
        $this->Change_database($this->config['base']['db']);
        $id = $this->escape($id);
        return $this->db->query("SELECT * FROM ".$this->config['prefix']."vote_time WHERE users = '$user' AND top = '$id'")->fetch_assoc();
    }
    ...
    {
        $this->Change_database($this->config['base']['db']);
        return $this->db->query("SELECT * FROM ".$this->config['prefix']."vote WHERE  id = '$id'")->fetch_assoc();
    }
    ...
    {
        $this->Change_database($this->config['base']['db']);
        $stmt = $this->db->prepare("INSERT INTO ".$this->config['prefix']."vote_time SET users = ?, time_last = ?, top = ?");
    ...
    {
        $this->Change_database($this->config['base']['db']);
        $this->db->query("UPDATE ".$this->config['prefix']."vote_time SET time_last = '$time_last', action = '0' WHERE users = '$username' AND top = '$top'");
    }
     ...
     * @param $bonus
     */
    public function addBonusVote($username, $top = NULL, $bonus)
    {
        $this->Change_database($this->config['base']['db']);
        $this->db->query("UPDATE ".$this->config['prefix']."vote_time SET action = '1' WHERE users = '$username' AND top = '$top'");
        $this->db->query("UPDATE ".$this->config['prefix']."account_lk SET bonus_vp = '$bonus' WHERE users = '$username'");
    }
    ...
    {
        $this->Change_database($this->config['base']['auth']);
        return $this->db->query("SELECT * FROM `account_premium` WHERE id = '$id'")->fetch_assoc();
    }
     ...
     * @return array
     */
    public function getBonusvp($id)
    {
        $this->Change_database($this->config['base']['db']);
        return $this->db->query("SELECT bonus_vp FROM ".$this->config['prefix']."account_lk WHERE id = '$id'")->fetch_assoc();
    }
    ...
    public function insertVip($id, $time, $time_new)
    {
        $this->Change_database($this->config['base']['auth']);
        $one = 1;
        $stmt = $this->db->prepare("REPLACE INTO account_premium SET id = ?, setdate = ?, unsetdate = ?, premium_type = ?, active = ?");
     ...
     * @param $id
     */
     public function insertVipAll($id, $time, $time_new)
    {
        $this->Change_database($this->config['base']['auth']);
        return $this->db->query("INSERT INTO account_premium SET id = '$id', setdate = '$time', unsetdate = '$time_new', premium_type = '1', active = '1'");
    }
     ...
     * @return array
     */
    public function issetItem($item, $id)
    {
        $this->Change_database($this->config['base']['db']);
        $item = $this->escape($item);
        $id = $this->escape($id);
        return $this->db->query("SELECT * FROM ".$this->config['prefix']."shop WHERE id_item = '$item' AND id = '$id'")->fetch_assoc();
    }
     ...
     * @return array
     */
    public function issetItems($item)
    {
        $this->Change_database($this->config['base']['db']);
        $item = $this->escape($item);
        //$id = $this->escape($id);
     ...
     * @param $enchantments
     */
    public function itemCreate($item_guid, $id_item, $owner_guid, $count, $enchantments, $charges)
    {
        $this->Change_database($this->config['base']['char']);
        $stmt = $this->db->prepare("INSERT INTO item_instance SET guid = ?, itemEntry = ?, owner_guid = ?, count = ?, enchantments = ?, charges = ?");
        $stmt->bind_param('iiiiss', $item_guid, $id_item, $owner_guid, $count, $enchantments, $charges);
        $stmt->execute();
    }
     ...
     * @param $race
     */
    public function updateRace($char, $race)
    {
        $this->Change_database($this->config['base']['char']);
        $race = $this->escape($race);
        $char = $this->escape($char);
        $this->db->query("UPDATE characters SET race = '$race' WHERE name = '$char'");
    }
    ...
    public function updateVip($id, $time)
    {
        $this->Change_database($this->config['base']['auth']);
        $id = $this->escape($id);
        $this->db->query("UPDATE account_premium SET unsetdate = '$time', active = '1' WHERE id = '$id'");
    }

    /**
     * @param $id
     */
    public function updateVipAll($id, $time, $newtime)
    {
        $this->Change_database($this->config['base']['auth']);
        $id = $this->escape($id);
        $this->db->query("UPDATE account_premium SET setdate = '$time', unsetdate = '$newtime', active = '1' WHERE id = '$id'");
    }
    ...
    {
        $this->Change_database($this->config['base']['db']);
        return $this->db->query("SELECT * FROM as_vote_mmotop WHERE MONTH(time_vote) = MONTH(NOW()) AND YEAR(time_vote) = YEAR(NOW()) AND username = '$username' ORDER BY id DESC");
    }
     ...
     * @param $bonus
     */
    public function upBonus($id, $bonus)
    {
        $this->Change_database($this->config['base']['db']);
        $this->db->query("UPDATE ".$this->config['prefix']."account_lk SET bonus = bonus - $bonus  WHERE id = '$id'");
    }

    public function addGoldChar($guid, $money)
    {
        $this->Change_database($this->config['base']['char']);
        $this->db->query("UPDATE characters SET money = money + $money*1000  WHERE guid = '$guid'");
    }

    public function addArenaChar($guid, $arenaPoints)
    {
        $this->Change_database($this->config['base']['char']);
        $arenaPoints = (int) $arenaPoints;
        $this->db->query("UPDATE characters SET arenaPoints = arenaPoints + $arenaPoints*150  WHERE guid = '$guid'");
    }

    public function addHonorChar($guid, $totalHonorPoints)
    {
        $this->Change_database($this->config['base']['char']);
        $totalHonorPoints = (int) $totalHonorPoints;
        $this->db->query("UPDATE characters SET totalHonorPoints = totalHonorPoints + $totalHonorPoints*1500  WHERE guid = '$guid'");
    }
}